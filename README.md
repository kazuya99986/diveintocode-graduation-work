...
## 制作の目的
このアプリケーションはプログラミングスクールDIVE INTO CODE（機械学習エンジニアコース2107）の卒業課題として作成しました。初学者のため、高度な機能は備えておりませんが、レビュー分析に興味がある方にお使いいただければ幸いです。
...
説明変数: ポジティブレビュー、ネガティブレビューの頻出単語（抽出する単語数は推論データサイズにより増減します）  
目的変数: ラベル（`positive`(1) or `negative`(0))  

決定係数とは各説明変数（今回の場合は各単語）の結果への寄与度（影響度）を示す値です。  

-  絶対値の大きさ: 寄与度が高い
-  プラスの値   : ポジティブ判定へ寄与している
-  マイナスの値  : ネガティブ判定へ寄与している 

出現頻度の高さに加えて、決定係数の高い単語を認識することで、より良い分析に繋がると思っています。
